...
export class MessagingService {
  ...
  private async saveToken(userId: string, token: string): Promise<void> {
    try {
      const tokenDoc = doc(collection(db, 'users', userId, 'fcmTokens'), token);
        ...
        <div className={`${t.visible ? 'animate-enter' : 'animate-leave'} 
          ...
          <div className="flex border-l border-gray-200 dark:border-gray-700">
            <button
              ...
              justify-center text-sm font-medium text-indigo-600 dark:text-indigo-400 
              hover:text-indigo-500 dark:hover:text-indigo-300 focus:outline-none 
